...
{{-- 2. HERO BENTO GRID --}}
@if($showcase->count() >= 1)
<section class="py-8 lg:py-12 bg-background">
    <div class="mx-auto max-w-[1280px] px-5 sm:px-8">
        <div class="mb-10 max-w-2xl">
            ...
            @if($tenant->description)
                <p class="mt-3 text-foreground/50 text-base font-medium">{{ Str::limit($tenant->description, 120) }}</p>
            @endif
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-4 h-auto md:h-[500px]">
            @if($showcase->count() >= 3)
                ...
                {{-- Principal --}}
                <div class="md:col-span-7 relative group overflow-hidden rounded-[2.5rem] bg-surface bento-item cursor-pointer"
                    ...
                    <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent p-8 flex flex-col justify-end">
                        <span class="inline-flex items-center py-0.5 px-2 rounded-full text-xs font-medium bg-primary/10 text-primary mb-3 uppercase font-black tracking-widest">Destacado</span>
                ...
                <div class="md:col-span-5 grid grid-rows-2 gap-4">
                    ...
                    <div class="relative group overflow-hidden rounded-[2rem] bg-surface bento-item cursor-pointer"
...
<nav class="mt-10 mb-10">
    <div class="mx-auto max-w-[1280px] px-5 sm:px-8">
        <div class="bg-background/95 backdrop-blur-xl shadow-sm border border-foreground/10 rounded-full py-2.5 px-3 flex items-center gap-3 overflow-x-auto no-scrollbar">
        <button onclick="filterCategory('all')" data-cat="all"
                class="sc-cat-pill text-sm py-1.5 px-6 rounded-2xl font-medium transition-colors bg-primary text-primary-foreground hover:bg-primary/90 shrink-0">Todos</button>
        @if(isset($categories))
            @foreach($categories as $cat)
                <button onclick="filterCategory('{{ $cat->id }}')" data-cat="{{ $cat->id }}"
                        class="sc-cat-pill text-sm py-1.5 px-6 rounded-2xl font-medium transition-colors text-foreground/80 hover:bg-surface border border-foreground/10 whitespace-nowrap shrink-0">{{ $cat->name }}</button>
            @endforeach
        @endif
        </div>
    </div>
...
<section class="py-12 lg:py-20" id="productos">
    <div class="mx-auto max-w-[1280px] px-5 sm:px-8">
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 md:gap-8">
            ...
            <div class="sc-product-card group flex flex-col bg-background rounded-[1.5rem] shadow-sm border border-foreground/5 overflow-hidden" data-category="{{ $product->category_id ?? 'all' }}">
                <div class="relative aspect-[4/5] rounded-[1.35rem] overflow-hidden bg-surface m-3 transition-all duration-300 group-hover:shadow-2xl group-hover:-translate-y-1">
                    @if($img)
                    ...
                    <button onclick="addToCart({{ $product->id }}, '{{ addslashes($product->name) }}', {{ $product->price_usd ?? 0 }}, '{{ $img }}')"
                            class="sc-add-btn bg-primary text-primary-foreground">
                ...
                <div class="px-1">
                    ...
                    <div class="flex items-center justify-between mt-1">
                        ...
                        <div id="qty-row-{{ $product->id }}" class="flex items-center gap-1 bg-surface rounded-full px-2 py-1" style="display:none!important">
                            <button class="size-6 rounded-full bg-background flex items-center justify-center text-xs font-bold" onclick="changeQty({{ $product->id }}, -1)">−</button>
                            <span class="text-xs font-black min-w-[14px] text-center" id="qty-val-{{ $product->id }}">1</span>
                            <button class="size-6 rounded-full bg-primary text-primary-foreground flex items-center justify-center text-xs font-bold" onclick="changeQty({{ $product->id }}, 1)">+</button>
                        </div>
                    </div>
                    @endif
                </div>
        ...
        <div class="flex items-center gap-2 flex-wrap">
            @foreach($visiblePay as $key => $pm)
                ...
                $pmIcon = match($key) {
                    ...
                    'cashea'     => 'icon-[tabler--shopping-cart-dollar]',
                    'krece'      => 'icon-[tabler--trending-up]',
                    'wepa'       => 'icon-[tabler--device-mobile-dollar]',
                    'lysto'      => 'icon-[tabler--calendar-dollar]',
                    'chollo'     => 'icon-[tabler--tag]',
                    'zelle'      => 'icon-[tabler--bolt]',
                    'zinli'      => 'icon-[tabler--wallet]',
