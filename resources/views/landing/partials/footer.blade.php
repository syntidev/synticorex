    ...
    <div class="container mx-auto">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div>
                ...
                @if($tenant->slogan)
                    <p class="text-current/70 mb-4">{{ $tenant->slogan }}</p>
                @endif
                    ...
                    <div class="flex space-x-4 mt-4">
                        @if($customization->social_networks['instagram'] ?? null)
                            <a href="https://instagram.com/{{ ltrim($customization->social_networks['instagram'], '@') }}" 
                               target="_blank" 
                               class="text-current/70 hover:opacity-100 transition-colors">
            ...
            <div>
                ...
                <ul class="space-y-3 text-current/70">
                    @if($tenant->phone)
                        <li class="flex items-center">
                            <span class="mr-2">📞</span>
                            <a href="tel:{{ $tenant->phone }}" class="hover:opacity-100 transition-colors">
                                {{ $tenant->phone }}
                            </a>
                        </li>
                    @endif
                    ...
                    @if($tenant->email)
                        <li class="flex items-center">
                            ...
                            <a href="mailto:{{ $tenant->email }}" class="hover:opacity-100 transition-colors">
                                {{ $tenant->email }}
                            </a>
                        </li>
                    @endif
        ...
        {{-- Copyright --}}
        <div class="mt-12 pt-8 border-t border-current/10 text-center text-current/70 text-sm">
            <p>&copy; {{ date('Y') }} {{ $tenant->business_name }}. Todos los derechos reservados.</p>
            <p class="mt-2">
                Powered by <a href="https://syntiweb.com" target="_blank" class="text-current/80 hover:opacity-100 transition-colors">SYNTIweb</a>
            </p>
        </div>
    </div>
</footer>
