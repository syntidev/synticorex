        ...
        <div class="accordion accordion-border accordion-flush space-y-4">
            @if(isset($customization->faq_items) && count($customization->faq_items) > 0)
                @foreach($customization->faq_items as $index => $faq)
                    <div class="accordion-item bg-base-100 border border-base-200 rounded-[2rem] overflow-hidden shadow-sm hover:shadow-glow-primary transition-all duration-300" id="faq-item-{{ $index }}">
                        <button class="accordion-toggle inline-flex items-center justify-between text-start text-lg font-bold px-8 py-6 w-full text-base-content hover:text-primary transition-colors" 
                        ...
                        <div id="faq-collapse-{{ $index }}" class="accordion-content w-full overflow-hidden transition-[height] duration-300" aria-labelledby="faq-item-{{ $index }}" role="region">
                            <div class="px-8 pb-6 text-base-content/60 leading-relaxed border-t border-base-200 pt-4">
                                {{ $faq['answer'] }}
                            </div>
                        </div>
                    </div>
