...
<section id="delivery-info" class="py-12 bg-gradient-to-br from-primary/5 to-success/5">
    <div class="container mx-auto px-6 max-w-4xl">
        <div class="text-center mb-8">
            ...
            <p class="text-foreground/70 mt-2">Llevamos tu pedido hasta la puerta de tu casa</p>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            
            {{-- Tarjeta de Tiempo --}}
            <div class="bg-background rounded-xl p-6 shadow-sm border border-border">
                ...
                <p class="text-sm text-foreground/60">Promedio estimado</p>
            </div>
                    ...
                    <div class="space-y-2">
                        @foreach($deliveryZones->take(3) as $zone)
                            <div class="flex justify-between items-center text-sm">
                                <span class="text-foreground/80">{{ $zone['name'] }}</span>
                        ...
                        @if($deliveryZones->count() > 3)
                            <p class="text-xs text-foreground/50 italic">
                                +{{ $deliveryZones->count() - 3 }} zonas más
                            </p>
                        @endif
                    </div>
                @else
                    <p class="text-sm text-foreground/60">Cobertura amplia en la ciudad</p>
                @endif
            </div>
        </div>
