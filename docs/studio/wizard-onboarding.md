---
title: "Asistente de configuración"
description: "Cómo configurar tu página de Studio por primera vez"
---

Cuando creas tu cuenta en SYNTIstudio, el asistente te guía paso a paso. No tarda más de 5 minutos.

## Los 5 pasos

<Steps>
  <Step title="Nombre y tipo de negocio">
    Escribe el nombre de tu negocio y elige a qué se dedica (restaurante, barbería, tienda, etc.). Esto ayuda a SYNTIweb a sugerir los mejores colores y textos para ti.
  </Step>
  <Step title="Información de contacto">
    Tu número de WhatsApp, teléfono y ciudad. Con esto tus clientes pueden contactarte desde tu página.
  </Step>
  <Step title="Foto principal">
    Sube una foto de tu local, logo o producto estrella. Esta es la primera imagen que verán tus clientes. Puedes subirla después si no tienes una a mano.
  </Step>
  <Step title="Colores de tu negocio">
    Elige una paleta de colores. SYNTIweb te muestra opciones organizadas por tipo de negocio. Un toque y tu página cambia de color al instante.
  </Step>
  <Step title="Confirmar y publicar">
    Revisa el resumen y dale clic a **Publicar**. Tu página queda en línea de inmediato.
  </Step>
</Steps>

## Después del asistente

Tu página ya está publicada pero puedes seguir mejorándola:

- Agrega productos y servicios desde el panel
- Sube más fotos
- Personaliza cada sección
- Descarga tu QR para imprimir

<Tip>
  No esperes tener todo perfecto para publicar. Publica con lo básico y ve mejorando poco a poco. Un negocio con página básica es mejor que uno sin página.
</Tip>

## Ver también

- [Asistente en detalle](/studio/asistente-configuracion) — Todos los campos del wizard explicados
- [Panel de administración](/studio/dashboard) — Cómo editar tu página después del asistente
- [Paletas de color](/studio/paletas-de-color) — Las 30+ paletas disponibles
