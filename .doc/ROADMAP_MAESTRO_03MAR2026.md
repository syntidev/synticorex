...
### 22 MAR 2026 — Tareas nuevas agregadas al backlog
- [ ] Confeti canvas-confetti al finalizar wizard (paso final, una línea JS)
- [ ] Email automatizado: UserRegistered → WelcomeMail + eventos del sistema
- [ ] DECISIÓN PENDIENTE: deploy automático GitHub Actions vs script manual bash
- [ ] Landing mobile-first: acortar longitud, eliminar FEATURED, SYNTIfood→"menú digital", SYNTIcat→"catálogo/vitrina"
- [ ] Landing mobile: badges IA flotantes se desorganizan en móvil — fix position/z-index
- [ ] Dashboard: mover BlueprintDonut + RevenueChart a nueva página /admin/analytics
- [ ] Documentación: migrar de VitePress/Hostinger (docs.syntiweb.com) a sistema propio en DB (esquema blog)
- [ ] Enrutamiento tenants por IP directa mientras DNS no apunta a Vultr
- [ ] Email: configurar proveedor externo Resend o Brevo en .env (no requiere dominio)
- [ ] Migración company_settings: manual_rate_enabled, manual_usd_rate, manual_eur_rate
